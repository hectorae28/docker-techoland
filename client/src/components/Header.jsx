...
const Header = () => {
  ...
  return (
    <nav className="bg-primary fixed w-full z-20 top-0 left-0">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto h-24 md:h-28 lg:h-32 px-4">
        <Link href="/" className="flex items-center text-white">
          <div className="flex cursor-pointer text-white items-center">
            <img
              src="/logo.png"
              className="h-12 md:h-14 lg:h-16 mr-3 logo"
              alt="Techoland-Logo"
            />
          </div>
        </Link>
        ...
        <div
          className="hidden w-full lg:block md:w-auto h-full"
          id="navbar-default"
        >
          <ul className=" bg-transparent h-full font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 md:mt-0 md:border-0 md:dark:bg-gray-900 dark:border-gray-700 text-white">
            {road.map((item, index) => (
              <li
                key={index}
                className="pl-3 pr-4 rounded md:bg-transparent md:p-0 h-full py-6 flex items-center"
              >
                {item.link !== "/laminas" ? (
                  <Link href={item.link}>
                    <div className="cursor-pointer">
                      {item.name}
                      {item.name === "Carrito" && state?.cart?.length > 0 && (
                  ...
                  <div
                    className="flex relative h-full items-center cursor-pointer"
                    id="dropdownSelector"
                  >
                    <Link href={item.link}>
                    ...
                    <ul
                      id="dropdown"
                      className="bg-primary absolute space-y-2 top-24 right-0 rounded-b-lg hidden"
                    >
